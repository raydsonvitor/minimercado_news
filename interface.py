...
# TOPLEVEL 5    >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   CADASTRO DE CLIENTES
    ...
    def get_all_data_to_treeview(self, data):
        try:
            if data:
                ...
                if yon:
                    self.imprimir_conta_cliente(self.cliente_selecionado, self.customer_id, self.cliente_selecionado)
            else:
    ...
    def imprimir_conta_cliente(self, customer_id, customer_name):
        try:
            data = backend.get_all_data_from_customer_by_id(customer_id)[2:]
            ...
            total = 0
            for item in data:
                total += helper.format_to_float(data['price']) * helper.format_to_float(data['quantity'])
                nota.append(f'{data['product_name']}  {data['price']}  {data['quantity']}  {data['date'][10]}')
            nota.append("------------------------------------------")
            nota.append(f'Total: {total}')
            nota.append("************************")
